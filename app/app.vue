<template>
  <div class="min-h-screen w-full relative font-['Courier_New',monospace] bg-[#008080]">
    ...
    <div v-if="!showContent" class="fixed inset-0 z-50 flex flex-col items-center justify-end p-6 transition-all duration-700" 
      ...
      <div v-if="!isLoaded" class="w-full max-w-[600px] bg-[#c0c0c0] border-2 border-white border-b-black border-r-black p-1 mb-8 shadow-xl">
        <div class="bg-white border border-[#808080] h-6 relative flex items-center overflow-hidden">
          <div class="bg-[#008080] h-full transition-all duration-[3000ms] ease-linear" :style="{ width: loadingProgress + '%' }"></div>
          <span class="absolute w-full text-[10px] text-center font-bold"> 로     딩     중 ...  ( {{ loadingCount }} seconds)</span>
        </div>
      </div>
    ...
    <div v-else class="w-full max-w-[480px] bg-[#c0c0c0] p-1 border-2 border-white border-b-black border-r-black mx-auto mt-4">
